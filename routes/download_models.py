...
if os.environ.get('RENDER'):
    base_dir = "/opt/render/project/src/saved_models"
else:
...
models = {
    # Using a more reliable model for face emotion recognition
...
# Create fallback models in case the primary ones fail
fallback_models = {
    "face_emotion_model": "dima806/facial_emotions_image_detection",
    "face_emotion_processor": "dima806/facial_emotions_image_detection"
}
...
try:
    ...
    try:
        ...
        print(f"Error with primary face model: {e}")
        print("Trying fallback face model...")
        model = AutoModelForImageClassification.from_pretrained(fallback_models["face_emotion_model"])
        processor = AutoImageProcessor.from_pretrained(fallback_models["face_emotion_processor"])
